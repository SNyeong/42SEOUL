...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/05/15 14:34:23 by seungnle          #+#    #+#             */
/*   Updated: 2021/05/28 06:49:56 by seungnle         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	cast_rays(t_struct *data)
{
	...
	ray_id = 0;
	ray_angle = data->rotation_angle - (radian(FOV_ANGLE) / 2);
	while (ray_id < data->w_width)
	{
		cast_single_ray(ray_id, ray_angle, data);
		ray_angle += radian(FOV_ANGLE) / (float)data->w_width;
		ray_id++;
	}
}
